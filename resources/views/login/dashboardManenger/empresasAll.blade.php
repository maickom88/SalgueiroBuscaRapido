								 ...
								 <td>
									 <span class="custom-checkbox">
										 ...
										 <td>{{$empresa->permissions->users->email}}</td>
								 		 <td>{{$empresa->name}}</td>
										 <td id="modalDescricao{{$id+=1}}" data-description="{{$empresa->description}}">{{substr($empresa->description, 0, 20).'...'}}<div style="color:royalblue; cursor:pointer" onclick="pegaId({{$id}})">Ver mais</div></td>
